...
async def lifespan(app: FastAPI):
    logger.info(
        f"Starting app {settings.app_name} v{settings.version} in {settings.environment} mode."
    )
    yield
    logger.info(
        f"🛑 Shutting app {settings.app_name} v{settings.version} in {settings.environment} mode."
    )
